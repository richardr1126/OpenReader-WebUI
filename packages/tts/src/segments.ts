...
export function buildTtsSegmentSettingsJson(settings: TTSSegmentSettings): TTSSegmentSettings | string {
  ...
  return process.env.POSTGRES_URL ? canonical : stableStringify(canonical);
}
...
// `normalizeLocator` now lives in the pure `./locator` module so client bundles
// can use it without dragging in this file's server-only deps. Re-exported here
// for back-compat with existing `@openreader/tts/segments` importers.
export { normalizeLocator };
